...
# prediction_y = DataFrame([real_data, prediction], 
    # index = prices.index[data2.Electricity.count()-results3.shape:],
	# columns = ['ActualPrices','ModelPrices'])
# ----------------------------------------------------------------------
# FIGURE 12 : Residuals Out-of-the-Sample
# ----------------------------------------------------------------------
fig12 = plt.figure()
# comparative = DataFrame([real_data.values,prediction.values], index=real_data.index)
...
# ax11.set_xlim([real_data.index[0],real_data.index[-1]])
# ax1.plot( real_data.index, prediction)
# ax1.xaxis.set_major_locator(prices.index[data2.Electricity.count()-results3.shape:].year)
# ax11.set_ylim([40,65])
ax01.legend()
ax01.set_title('Actual prices vs. Out-The-Sample Prediction')
# ax11.xaxis.set_major_locator(prediction.index[1:].year)
# ax1.plot(prediction_y.ix[2009].values)
# ax1.plot(prediction_y.ix[2010].values)
# ax1.plot(prediction_y.ix[2011].values)
# ax1.plot(prediction_y.ix[2012].values)

# ax21 = fig1.add_subplot(4,1,2)
# ax21.plot( prediction.index, prediction.values, label = 'Prediction')
# ax11.set_xlim([real_data.index[0],real_data.index[-1]])
# ax21.set_ylim([min(real_data),max(prediction)])
# ax21.legend()
# ax21.set_title('Out-The-Sample prediction')

ax02 = fig12.add_subplot(3,1,2)
ax02.plot(residual_out.index, residual_out.values ,'ro--', label = 'Residual out-the-sample')
# ax2.plot(residual_ots.ix[2009].values, color = 'c',label='2009')
# ax2.plot(residual_ots.ix[2010].values, color = 'b',label='2010')
# ax2.plot(residual_ots.ix[2011].values, color = 'k',label='2011')
# ax2.plot(residual_ots.ix[2012].values, color = 'r',label='2012')
ax02.legend()
...
# ax13.hist(residual_out,bins=25, alpha = 0.2, color='c', normed=True,label='2009')
# ax3.hist(residual_out.ix[2010].values,bins=25, alpha = 0.2, color='b', normed=True,label='2010')
# ax3.hist(residual_out.ix[2011].values,bins=25, alpha = 0.2, color='k', normed=True,label='2011')
# ax3.hist(residual_out.ix[2012].values,bins=25, alpha = 0.2, color='r', normed=True,label='2012')
# ax13.hist(residual_out, bins=25)
# ax3.legend()
...
# ax21.hist(residual_ots.ix[2009].values,bins=25, alpha = 0.2, color='c', normed=True)
# ax1.set_title('2009')
...
# res_yy = residual
# res_year = res_y.set_index(residual_out.index.year)
# res_year = res_y.groupby(level = 0)
...
def position(prces, resid, num_std, ma_resid, std_resid):
	...
	for k in range(1,n-1):
		...
		# Profit Taking level for Short-Positions
		
		elif position_f[k-1] < 0 and ( (- position_f[k-1] - prces[k-1])> 2 or (- position_f[k-1] - prces[k-1]) < -1): 
			
			results[k] = - position_f[k-1] - prces[k] 	# Closing Short-position
			position_f[k] = 0
			position_m2m[k] = position_m2m[k-1] + prces[k-1] - prces[k]
		
		# Profit Taking level for Long-Positions
		
		elif position_f[k-1] > 0 and ((prces[k-1] - position_f[k-1]) > 2 or (prces[k-1] - position_f[k-1]) < -1): 	
	...
	return results, position_m2m
	
rslt09, pos_m2m09 = position(pr_year.values[2009], res_09[:,0], num_std, ma_res_09, std_res09)	
rslt10, pos_m2m10 = position(pr_year.values[2010], res_10[:,0], num_std, ma_res_10, std_res10)
rslt11, pos_m2m11 = position(pr_year.values[2011], res_11[:,0], num_std, ma_res_11, std_res11)
rslt12, pos_m2m12 = position(pr_year.values[2012], res_12[:,0], num_std, ma_res_12, std_res12)
# ----------------------------------------------------------------------
# FIGURE 14 : Final Backtesting results
# ----------------------------------------------------------------------
fig14 = plt.figure()
