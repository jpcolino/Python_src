...
results1 = by_year.apply(regress, 'Electricity', ['Carbon'])
results2 = by_year.apply(residual, 'Electricity', ['Carbon'])
...
for i in seq_years : 
	...
	# predict_out 
	for j in year_seq :
		predict_out[j] = results1.ix[i]['intercept'] \
		+ results1.ix[i]['Carbon']*year_p[j,3] 
...
ax03.plot( ma_res_11 - num_std*std_res11, 'r--', label='MA25-STD25_resid' )
ax03.plot( res_11[:,0], 'ro--', label='MA25_resid' )
ax05 = ax03.twinx()
...
ax04.plot( ma_res_12 - num_std*std_res12, 'r--', label='MA25-STD25_resid' )
ax04.plot( res_12[:,0], 'ro--', label='MA25_resid' )
ax05 = ax04.twinx()
...
def position(prces, resid, num_std, ma_resid, std_resid):
	...
	for k in range(1,n-1):
		...
		# CLOSING OLD & OPENING NEW POSITION ----------------------------------------------------------------------------
		
		# Closing a SHORT position AND Opening a LONG position (from a BEARISH mkt <-> resid > 0)
		if position_f[k-1] < 0 and resid[k] < ma_resid[k] - num_std * std_resid[k]  :
			results[k] = - position_f[k-1] - prces[k] 	
			position_f[k] = prces[k]
			position_m2m[k] = position_m2m[k-1] + prces[k-1] - prces[k]
		...
		# Closing LONG position	AND Opening a SHORT position (from a BULLISH mkt <-> resid < 0)
		elif position_f[k-1] > 0 and resid[k] > ma_resid[k] + num_std * std_resid[k]  :	
...
rslt13_s, pos_m2m13_s, pos_f13_s = position_stop(pr_year.values[2013], res_13[:,0], num_std, ma_res_13, std_res13)
...
# ----------------------------------------------------------------------
# FIGURE 16-17-18-19 : Final Backtesting results per year
# ----------------------------------------------------------------------
plt.rc('axes', grid=True)
...
# ----------------------------------------------------------------------
# FIGURE 21 : Final Empirical Distribution of the P&L
# ----------------------------------------------------------------------
...
fig21 = hist(results_cum.values, bins=10, alpha = 0.2, color='g', normed=True)
