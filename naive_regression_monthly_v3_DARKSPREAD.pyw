...
results1 = by_year.apply(regress, 'Electricity', ['Coal','Carbon'])
results2 = by_year.apply(residual, 'Electricity', ['Coal','Carbon'])
...
for i in seq_years : 
	...
	# predict_out 
	for j in year_seq :
		predict_out[j] = results1.ix[i]['intercept'] + results1.ix[i]['Coal'] * year_p[j,1] \
...
def position(year, p_power, p_gas, p_coal, p_carbon, resid, num_std, ma_resid, std_resid):
	...
	for k in range(1,n-1):
		...
		if position_f[k-1] < 0 and resid[k] > 0 and resid[k] > ma_resid[k] - num_std * std_resid[k]  :
			...
			position_m2m[k] = position_m2m[k-1] + p_power[k-1]  \
			    - results1.Carbon.ix[year] * p_carbon[k-1] \
			    - p_power[k] + results1.Carbon.ix[year] * p_carbon[k]	
		...
		# WAITING in a LONG position (from a BULLISH mkt <-> resid < 0)
			...
			position_m2m[k] = position_m2m[k-1] + p_power[k] \
			    - results1.Carbon.ix[year] * p_carbon[k] \
			    - p_power[k-1] + results1.Carbon.ix[year] * p_carbon[k-1]			
		...
		if position_f[k-1] < 0 and resid[k] < ma_resid[k] - num_std * std_resid[k]  :
			results[k] = - position_f[k-1] - p_power[k] \
			    + results1.Carbon.ix[year] * p_carbon[k]  
			position_f[k] = 0
			position_m2m[k] = position_m2m[k-1] + p_power[k-1]  \
			    - results1.Carbon.ix[year] * p_carbon[k-1] \
